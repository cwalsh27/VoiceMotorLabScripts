...
for filename in os.listdir():
    if filename.endswith(".xlsx"):
        ...
        for col in range(1, 66):  # 66 is just three iterations of the alphabet, number is arbitrary
            if ws.cell(1, col).value.endswith('.RESP') and 'welcome' not in ws.cell(1, col).value:
            ...
            if ws.cell(1, col).value.endswith('.OnsetTime') and 'welcome' not in ws.cell(1, col).value:
